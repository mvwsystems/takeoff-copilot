// TEMPORARY — delete after use. Sends the free-beta invitation template to the
// operator's inbox so they can review/forward it. Guarded by the fn secret.
// Blue brand (#0057FF) + bgcolor attributes so Gmail renders the header/button.
const A = '#0057ff'
...
const html = '<html><body style="margin:0;padding:0;background:#e7ebf2;">'
  ...
  + '<p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#3a4250;">Hey [FIRST NAME],</p>'
...
export const handler = async (event) => {
  ...
  const to = (JSON.parse(event.body || '{}').to) || 'hello@6signal.co'
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: 'TEMPLATE — Free beta invitation (forward to testers)',
      html,
    }),
  })
